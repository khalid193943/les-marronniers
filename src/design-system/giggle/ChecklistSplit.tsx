...
export const ChecklistSplit: React.FC<ChecklistSplitProps> = ({
  ...
  return (
    <div className={`max-w-6xl mx-auto px-4 sm:px-8 grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 items-center ${className}`}>
      ...
      >
        <span className="inline-flex items-center gap-2 rounded-full bg-[#084274]/8 px-4 py-1.5 text-xs font-bold text-[#084274] mb-5">
          <span className="w-1.5 h-1.5 rounded-full bg-[#e3a044]" />
          {tag}
        </span>
        <h2 className="font-heading text-3xl sm:text-4xl text-[#084274] leading-tight tracking-tight mb-4">
          {title}
        </h2>
        {line && (
          <p className="font-body text-base text-[#084274]/70 leading-relaxed mb-7 max-w-md">
            {line}
          </p>
        )}
        ...
        <ul className="flex flex-col gap-3.5 mb-8">
          {items.map((it, idx) => (
            ...
            >
              <span className="w-5 h-5 rounded-full bg-[#e3a044] text-[#084274] flex items-center justify-center text-[11px] font-bold shrink-0 mt-0.5">
                ✓
              </span>
              <span className="font-body text-[15px] text-[#084274]/85 leading-snug">{it}</span>
            </motion.li>
          ))}
        </ul>
        ...
        {ctaLabel && onCta && (
          <button
            onClick={onCta}
            className="inline-flex items-center gap-3 rounded-full bg-[#084274] text-[#feeddb] font-body font-bold text-sm pl-6 pr-2 py-2 cursor-pointer hover:bg-[#05335b] transition-colors group"
          >
            <span>{ctaLabel}</span>
